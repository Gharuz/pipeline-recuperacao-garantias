...
# DESC: Higienização de dados e auditoria de integração (Controlo Interno vs. Portal CAT)
...
def executar_pipeline():
    logging.info("Carregando camada bronze (backlog bruto)...")
    ...
    colunas_esperadas = [
        ...
        'data_abertura',
        'valor_pleiteado',
        'id_processo_cat',  # Atualizado: Substitui doc_contabil_sap
        'dias_em_aberto'
    ]
    ...
    # Remoção de valores não auditáveis (zeros e negativos)
    df = df.loc[df['valor_pleiteado'] > 0].copy()

    # 3. Identificação de Falhas de Integração (Portal Fabricante)
    logging.info("Identificando falhas de integração com o Portal CAT...")

    # Atualizado: Procura nulos ou erros de sistema na coluna do fabricante
    integracao_fail_mask = (
        df['id_processo_cat'].isna() |
        (df['id_processo_cat'] == 'ERRO_INTEGRACAO') 
    )
    ...
    logging.info(f"Total de registos válidos: {total_registros}")
    logging.info(f"Falhas de integração (ausência no portal CAT): {falhas_integracao}")
    logging.info(f"Taxa de falha de integração: {taxa_falha_integracao:.2%}")

    # 7. Persistência da Camada Prata
    logging.info("A guardar camada silver...")
